...
func GetNewDomainSection(workdir string, category string) string {
...
reset:
	...
	domain, err := utils.InputAfterQuestion("Enter the domain you want to delete: ")
	if err != nil {
		log.Fatalln(err)
	}
	err = utils.CheckIsValidDomain(domain)
	...
	isDomainNotExist, _ := utils.CheckPathIfNotExist(workdir + "/internal/" + category + "/" + domain)
	if isDomainNotExist {
		views.DisplayResetWithMessage("domain not found", constants.ColorYellow)
		goto reset
	}
...
reset2:
	...
	if strings.ToLower(confirm) != "n" && strings.ToLower(confirm) != "y" {
		views.DisplayClear()
		views.DisplayTitle()
		fmt.Println(("Enter the domain you want to delete: " + domain))
		goto reset2
	} else if strings.ToLower(confirm) == "n" {
		views.DisplayClear()
		goto reset
